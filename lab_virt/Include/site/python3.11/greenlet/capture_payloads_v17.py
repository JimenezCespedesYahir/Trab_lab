...
# - /querydata requests and responses are persisted
#
# ============================================================
...
OUTPUT_DIR = Path("capture_payloads_v17_outputs")
REQUEST_DIR = OUTPUT_DIR / "querydata_requests"
RESPONSE_DIR = OUTPUT_DIR / "querydata_responses"
DEBUG_DIR = OUTPUT_DIR / "debug"

for directory in (OUTPUT_DIR, REQUEST_DIR, RESPONSE_DIR, DEBUG_DIR):
...
def handle_request(request) -> None:
    ...
    try:
        ...
        last_querydata_at = time.monotonic()
        ...
        save_jsonish(
            ...
            post_data,
            {
                ...
                "event": querydata_event_counter,
            },
        )

    except Exception as exc:
        log(f"REQUEST CAPTURE ERROR: {exc}")
...
def handle_response(response) -> None:
    ...
    try:
        ...
        log(f"RESPONSE CAPTURE ERROR: {exc}")
...
def find_active_overlay(
    ...
    while time.monotonic() < deadline:
        ...
        if best and best_score >= 45 and (best.get("optionCount") or best.get("searchCount") or best.get("text")):
            log(f"ACTIVE OVERLAY SELECTED: {best.get('id')} score={best_score:.1f}")
            return OverlayRef(
                page=page,
    ...
    if best:
        log(f"FALLBACK OVERLAY SELECTED: {best.get('id')} score={best_score:.1f}")
        return OverlayRef(
            page=page,
            overlay_id=str(best["id"]),
            owner_label=owner_label,
...
def reset_overlay_scroll(overlay: OverlayRef) -> None:
    try:
        ...
        overlay.page.wait_for_timeout(400)
    except Exception:
        pass
...
def scroll_overlay(overlay: OverlayRef, delta: int = 220) -> Dict[str, Any]:
    ...
    try:
        ...
        overlay.page.wait_for_timeout(500)
        return result
    except Exception as exc:
        log(f"SCROLL FAILED: {exc}")
        overlay.page.mouse.wheel(0, delta)
        overlay.page.wait_for_timeout(450)
...
def click_option(overlay: OverlayRef, text: str, max_scrolls: int = 100) -> None:
    ...
    for scroll_index in range(max_scrolls):
        ...
        log(f"SCROLL ITERATION {scroll_index + 1}: OPTIONS={len(options)}")
        ...
        for option in options:
            ...
            log(f"OPTION score={score:.1f} selected={option.get('selected')} text={normalized_option[:120]}")
        ...
        if scored:
            ...
            if best_score >= 78:
                log(f"MATCH FOUND score={best_score:.1f}: {best.get('text')}")
...
def apply_filter(page: Page, label: str, search_value_text: str, expected_option: str, attempts: int = 3) -> None:
    ...
    for attempt in range(1, attempts + 1):
        try:
            log(f"APPLY ATTEMPT {attempt}/{attempts}")
...
def reset_grupo_producto(page: Page) -> None:
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_error = exc
            log(f"RESET GRUPO PRODUCTO FAILED attempt={attempt}: {exc}")
            close_overlay(page, strict=False)
            wait_for_dom_quiet(page, quiet_ms=800, timeout_ms=6000)
